...
func newStorage(t *testing.T) (*REST, *BindingREST, *StatusREST, storagefactory.TestClientServer) {
	internalStorage, server := registrytest.NewStorage(t, factory, "")
	restOptions := generic.RESTOptions{Storage: internalStorage, Decorator: generic.UndecoratedStorage, DeleteCollectionWorkers: 3}
...
func newFailDeleteStorage(t *testing.T, called *bool) (*REST, storagefactory.TestClientServer) {
	internalStorage, server := registrytest.NewStorage(t, factory, "")
	failDeleteStorage := FailDeletionStorage{internalStorage, called}
...
func TestStorageCreateBindingNoPod(t *testing.T) {
	storage, bindingStorage, _, server := newStorage(t)
	defer server.Terminate(t)
	ctx := api.NewDefaultContext()
...
func TestStorageCreateFailsWithoutNamespace(t *testing.T) {
	storage, _, _, server := newStorage(t)
	defer server.Terminate(t)
	pod := validNewPod()
	pod.Namespace = ""
...
func TestStorageCreateWithContainersNotFound(t *testing.T) {
	storage, bindingStorage, _, server := newStorage(t)
	defer server.Terminate(t)
	ctx := api.NewDefaultContext()
	key, _ := storage.KeyFunc(ctx, "foo")
...
func TestStorageCreateWithConflict(t *testing.T) {
	storage, bindingStorage, _, server := newStorage(t)
	defer server.Terminate(t)
	ctx := api.NewDefaultContext()
...
func TestStorageCreateWithExistingContainers(t *testing.T) {
	storage, bindingStorage, _, server := newStorage(t)
	defer server.Terminate(t)
	ctx := api.NewDefaultContext()
	key, _ := storage.KeyFunc(ctx, "foo")
...
func TestStorageCreateBinding(t *testing.T) {
	ctx := api.NewDefaultContext()
...
func TestStorageUpdateNotScheduled(t *testing.T) {
	storage, _, _, server := newStorage(t)
	defer server.Terminate(t)
	ctx := api.NewDefaultContext()
...
func TestStorageUpdateScheduled(t *testing.T) {
	storage, _, _, server := newStorage(t)
	defer server.Terminate(t)
	ctx := api.NewDefaultContext()
...
func TestStorageUpdateStatus(t *testing.T) {
