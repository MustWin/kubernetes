...
func newStorage(t *testing.T) (*Storage, storagefactory.TestClientServer, allocator.Interface) {
	internalStorage, server := registrytest.NewStorage(t, factory, "")
	mem := allocator.NewAllocationMap(100, "rangeSpecValue")
	storage := NewStorage(mem, "/ranges/serviceips", api.Resource("serviceipallocations"), internalStorage)
	return storage, server, mem
}
...
func TestStore(t *testing.T) {
	...
	other = allocator.NewAllocationMap(100, "rangeSpecValue")
	otherStorage := NewStorage(other, "/ranges/serviceips", api.Resource("serviceipallocations"), storage.storage)
